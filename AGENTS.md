...
- For any UI change, validate the change with Playwright before finalizing.
- Include at least one concrete Playwright check (navigation + interaction and/or screenshot verification) for the modified area.

## Change Scope Requirement
- Do not make behavior changes unless explicitly requested by the user.
- Do not remove or replace existing behavior unless the user explicitly asks for that removal/replacement.
